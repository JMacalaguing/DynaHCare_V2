...
export default function Home() {
  ...
  // Calculate Fully Immunized Quarterly Totals
const calculateFullyImmunizedQuarterlyTotals = (monthlyCounts: number[]) => {
  if (!Array.isArray(monthlyCounts) || monthlyCounts.length !== 12) {
    console.error("Invalid monthly counts data:", monthlyCounts);
    return {
      firstQuarter: 0,
      secondQuarter: 0,
      thirdQuarter: 0,
  ...
  return {
    firstQuarter: monthlyCounts.slice(0, 3).reduce((sum, val) => sum + val, 0), // January to March
    secondQuarter: monthlyCounts.slice(3, 6).reduce((sum, val) => sum + val, 0), // April to June
    thirdQuarter: monthlyCounts.slice(6, 9).reduce((sum, val) => sum + val, 0), // July to September
    fourthQuarter: monthlyCounts.slice(9, 12).reduce((sum, val) => sum + val, 0), // October to December
  };
};

const TARGET_PER_QUARTER = 171;
const quarterlyTotal = calculateFullyImmunizedQuarterlyTotals(monthlyFullyImmunizedCounts);
